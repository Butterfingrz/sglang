...
class RequestStage(str, enum.Enum):
    ...
    PREFILL_PREPARE = "prefill_prepare"                     #? 准备 prefill 任务，初始化请求
    PREFILL_BOOTSTRAP = "prefill_bootstrap"                 #? 完成引导初始化，准备开始前向计算
    PREFILL_FORWARD = "prefill_forward"                     #? 执行 prefill 前向计算
    PREFILL_TRANSFER_KV_CACHE = "prefill_transfer_kv_cache" #? 将 KV Cache 传输到 decode 节点

    # disaggregation decode
    DECODE_PREPARE = "decode_prepare"
    DECODE_BOOTSTRAP = "decode_bootstrap"
    DECODE_WAITING = "decode_waiting"                      #? 等待进入 decode 调度队列
    DECODE_TRANSFERRED = "decode_transferred"              #? KV Cache 传输完成，可以开始解码
